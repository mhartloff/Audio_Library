...
define(function (require) {
	...
	var PannerSound = require("PannerSound");
	...
	function EchoSound(source, scene) {
		...
		this.offset = new Vector3(0, 0, 0);		// Offset of the sound source from the position.
	...
	EchoSound.prototype.setOffset = function (offset /* Vector3 */) {
	...
	EchoSound.prototype.play = function () {
		...
		for (var index = 0; index < echoObjects.length; index++) {
			...
			if (true) {
				...
				var distance1 = sourceSound.soundOrigin.distance(new Vector3(echoPoint.x, 0, echoPoint.y));
				var echoSound = new PannerSound(this.source);
				echoSound.setPosition(new Vector3(echoPoint.x, 0, echoPoint.y));
