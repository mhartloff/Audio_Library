...
define(function (require) {
	...
	function Scene() {
		...
		this.position = new Vector3();	// The position of the player
		...
		this.setOrientationAxes(new Vector3(-1, 0, 0), new Vector3(0, 1, 0), new Vector3(0, 0, -1));
	...
	{
		this.setPlayerDirection(new Vector3(Math.cos(dir), 0.0, Math.sin(dir)));
	}

	// Set a player's direction on the xz plane.
	Scene.prototype.setPlayerDirection = function (dir /* Vec3 */)  {
		this.setOrientationAxes(dir.cross(new Vector3(0, 1, 0)), new Vector3(0, 1, 0), dir);
	}
	...
	Scene.prototype.onMouseDown = function (x, z) {
		...
		var clickLoc = new Vector3(x, 0, z);
		for (var id in this.objects) {
			if (this.objects.hasOwnProperty(id)) {
				var obj = this.objects[id];
	...
	Scene.prototype.onMouseMove = function (x, z) {
		...
		if (this.selectedObject) {
			this.selectedObject.setPosition(new Vector3(x, 0, z));
		}
	}
	...
	Scene.prototype.onKeyDown = function (keyCode, x, z) {
		...
		switch (keyCode) {
			case 37: {   // left arrow
				this.movePlayer(new Vector3(0.2, 0, 0));
				handled = true;
				break;
			}
			case 38: case 87: {   // up arrow, W
				this.movePlayer(new Vector3(0, 0, 0.2));
				handled = true;
				break;
			}
			case 39: {   // right arrow
	...
	Scene.prototype.updateTouchMovement = function (interval /* in ms */) {
		...
		if (this.moveMode == "forward" && magnitude >= this.deadZoneRadius) {	
			...
			if (magnitude > 0) {
				...
				this.movePlayer(new Vector3(0.0, 0.0, moveDistance));
				...
				if (Math.floor(this.pedometer / this.stepSize) > this.numSteps) {
					...
					this.playerObject.play(WebAudio.getSoundSource("stepForward"), false, 0, { offset: new Vector3(this.numSteps % 2 == 1 ? -0.1 : 0.1, -2.5, 0) });
				}
			}
		}
		if (this.moveMode == "back" && this.backRange.contains(angle)) {
			...
			if (magnitude > 0) {
				...
				if (Math.floor(this.pedometer / this.stepSize) > this.numSteps) {
					this.numSteps = Math.floor(this.pedometer / this.stepSize);
					this.playerObject.play(WebAudio.getSoundSource("stepBack"), false, 0, { offset: new Vector3(this.numSteps % 2 == 1 ? -0.1 : 0.1, -2.5, 0) });
				}
			}
		}
	}
